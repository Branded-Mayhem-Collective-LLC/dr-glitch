...
export function parseInvocation(args) {
  ...
  assert(['--rehearsal', '--rehearsal-worktree', '--live', '--live-telemetry'].includes(mode), 'Choose an explicit rehearsal or live mode; there is no default network target.');
  const live = mode === '--live' || mode === '--live-telemetry';
  assert.equal(args.length, live ? 2 : 1, 'Live mode requires exactly one deployment receipt; rehearsal takes no receipt or URL.');
  if (live) assert(typeof receiptPath === 'string' && receiptPath.trim(), 'Deployment receipt path is required.');
  return { live, origin: live ? LIVE_ORIGIN : REHEARSAL_ORIGIN, receiptPath, ...(mode === '--rehearsal-worktree' ? { worktreeRehearsal: true } : {}), ...(mode === '--live-telemetry' ? { liveTelemetry: true } : {}) };
}
