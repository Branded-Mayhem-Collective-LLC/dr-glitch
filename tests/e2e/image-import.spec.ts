...
test("invalid and stale artwork selections preserve the latest valid source", async ({ page }) => {
  ...
  // Default layout docks Layers; the upload card lives in the Select panel,
  // so activate it before capturing the input or asserting the card name.
  await activateTool(page, "select");
  const input = artworkFileInput(page);
  ...
  await expect(uploadCardName(page)).toHaveText("latest.png");
...
test("latest registration import wins and output reset retains an in-flight valid asset", async ({ page }) => {
  await page.addInitScript(() => {
  ...
  const input = registrationFileInput(page);
  ...
  await expect(drawer(page, "output").getByText("latest.svg", { exact: true })).toBeVisible();
  await expect(page.getByRole("status")).toContainText("Registration mark loaded: latest.svg");
  await input.setInputFiles({ name: "slow.svg", mimeType: "image/svg+xml", buffer: svg });
  await exportPanel(page).getByRole("button", { name: /Reset/ }).click();
  await page.evaluate(() => (window as unknown as { releaseMark: () => void }).releaseMark());
  await page.waitForFunction(() => Number(document.documentElement.dataset.loadedImages) >= 6);
  await page.evaluate(() => new Promise(requestAnimationFrame));
  await expect(page.getByRole("status")).toContainText("Registration mark loaded: slow.svg");
  await expect(page.getByRole("button", { name: "Replace registration SVG" })).toBeVisible();
});
